...
public class StatementService {	//Change the name
	...
	public List<TransactionDTO> statementBetweenDates(TransactionDTO recievedtxnDto) {
		
		List<Transaction> txnlist= transactionDao.fetchStatementBetweenDate(recievedtxnDto.getFrmAccNo(),recievedtxnDto.getFromDate(),recievedtxnDto.getEndDate());
		List<TransactionDTO> txndtolist=new ArrayList<TransactionDTO>();
		for(Transaction txn:txnlist) {
			TransactionDTO txndto=new TransactionDTO(txn);
			txndtolist.add(txndto);
